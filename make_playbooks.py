...
def make_playbook(pb_name, pb_list):
    ...
    if '_teaser' not in pb_name and '_gm' not in pb_name:
        parsed_moves = sum([parse_moves(staged_name(pbs)) for pbs in pb_list], [])
        all_moves = markup_moves(parsed_moves) # parse all the moves and put them in a single list together
        
        with open(outfile_xml, 'w', encoding='utf8') as xml_out: # write it out as XML to build the PDF playbooks.
            xml_out.write(render_xml(parsed_moves))
